...
def bb():
    d = {'kiwi': ['kiwi.good.svg'], 'apple': ['apple.good.2.svg',
                                              'apple.good.1.svg'], 'banana': ['banana.1.ugly.svg', 'banana.bad.2.svg']}
    ...
    for fruit in d:
        ...
        for state in "good", "bad", "ugly":
            html += "<td>{}</td>".format('<br>'.join(
                f for f in d[fruit] if ".{}.".format(state) in f))
        html += "</tr>"
    html += "</table></html>"
    print('html page')
    print(html)
...
def html_page_body_start():
    ...
    <body>
        <header>
            <h1>Tableau Périodique des Eléments</h1>
            <p>Generated from pyhon sctipt</p>
            <p><small>CERCO IA, Piscine Python Day01 ex08</small></p>
            <br />
        </header>
    """
...
def html_page_body_table(elements_dict):
    ...
    page_body_table = page_body_table_start + \
        page_body_table_body + page_body_table_end
...
def html_body_footer():
    body_footer = """
        <footer>
	        <div id="footer">
                <p>(C) 2020, CERCO IA, Tous droits réservés.</p>
            </div>
	    </footer>
    """
    return body_footer
...
def main(args):
    ...
    with open("periodic_table.html", "w") as f_out:
        for line in m:
            f_out.write(line)
        print(m)
